...
const ConnectWalletModal = ({
  ...
  const { closeWeb3Modal } = useWeb3Modal();
  const fulfillConnectRequest = async (connector: any) => {
    try {
      console.log({ connector });
      const resp = await connectAsync({ connector });
      // callbackForData({ accountAddress: resp.account.toString(), chainId: resp.chain.id });
      callbackForData.resolve({
        accountAddress: resp.account.toString(),
        chainId: resp.chain.id,
      });
      onModalClose();
      return;
    } catch (error: any) {
      ...
      console.log({ error });
      onModalClose();
      callbackForData.reject();
      return;
    }
  };
  const connectorToDisplay = (connector: any) => {
          ...
          <div className="p-6">
            ...
            <ul className="my-4 space-y-3">
              {connectors.map((connector) => {
                const { name, imgSrc } = connectorToDisplay(connector);
                return (
                  <button
                    className="flex flex-row items-center p-4 rounded-[12px] text-black hover:bg-white/10 linear duration-300"
                    // disabled={!connector.ready}
                    key={connector.id}
                    onClick={() => fulfillConnectRequest(connector)}
                  >
                    <Image
                      src={imgSrc}
                      width={50}
                    ...
                    <span className="flex-1 ml-3 text-white whitespaceNowrap">
                      {name}
                    </span>
                  </button>
                );
